...
export class LoadCtrl extends SceneCtrl {
  ...
  public initLink() {
    let view = this.view;
    let progress_bar = <Laya.ProgressBar>zutil.getElementsByName(view, 'progress_bar')[0];

    let con = zutil.getElementsByName(view, 'con')[0];
    let bg_ani = zutil.getElementsByName(view, 'bg_ani')[0] as laya.ani.bone.Skeleton;
    let cover = zutil.getElementsByName(view, 'cover')[0];
    let cover_anis = zutil.getElementsByName(view, 'cover_ani') as laya.ani.bone.Skeleton[];
  ...
  */
  private loadingQueue() {
    ...
    /**加载完成执行函数*/
    let loaded_callback: Laya.Handler = Laya.Handler.create(this, new_callback);

    if (load_style == 'show_loading') {
      if (!this.is_showing) {
        this.show();
      }
      loading_callback = Laya.Handler.create(this, this.showLoadingProgress, null, false);
    }
  ...
  private checkHide(loading_item) {
    ...
    setTimeout(() => {
      ...
      if (this.loading_item && this.loading_item.load_style == 'show_loading') {
        hide_load = false;
      }
      if (hide_load) {
        this.onLoaded();
   ...
   */
  private loadItem(res: any[], loaded_callback: Laya.Handler, loading_callback: Laya.Handler) {
    /**空的资源列表 直接执行返回函数 */
    if (!res.length) {
      loaded_callback.run();
      return;
    ...
    if (loading_callback) {
      loading_progress_callback = Laya.Handler.create(this, (progress) => {
        loading_callback.runWith((loaded_item_num + progress) / len);
      }, null, false);
    }

    for (let i = len - 1; i >= 0; i--) {
      this.loadItem(res[i], Laya.Handler.create(this, () => {
        loaded_item_num++;
        if (loading_callback) {
    ...
    if (res_info.is_scene) {
      for (let len = load_queue.length, i = len - 1; i >= 0; i--) {
        ...
        if (load_queue[i].order <= res_info.order) {
          zutil.logErr(`load:${res_info.ctrl_name}--${load_queue[i].ctrl_name}: end_listener close other`);
          load_queue[i].end_listener = undefined;
          load_queue[i].load_style = 'in_background';
        } else {
          zutil.logErr(`load:${res_info.ctrl_name}--${load_queue[i].ctrl_name}: end_listener close self`);

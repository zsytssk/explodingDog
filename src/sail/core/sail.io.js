; (function () {
	...
	var IOPrimus = (function () {
		...
		_proto.onData = function (data) {
			...
			// if(parsedData.cmd == "sync"){
			// 	(parsedData.res.bb.length != 0 || parsedData.res.bd.length != 0) && console.log("命令：" + parsedData.cmd + "\n<<<=====接收到'IO-Primus'数据, 时间:" + Date.now() + "\n" + dataString);
			// }else{
			Sail.DEBUG && console.log("命令：" + parsedData.cmd + "\n<<<=====接收到'IO-Primus'数据, 时间:" + Date.now() + "\n" + dataString);
			// }
			this.callback(parsedData.cmd, parsedData.res != null || parsedData.res != (void 0) ? parsedData.res : parsedData.rep, parsedData.code, parsedData.error || parsedData.msg);
		}
	...
	(function () {
		...
		_proto.dispatch = function (cmd, data, code, errormsg, type) {
			...
			if (cmd != "io.reconnect" && cmd != "io.open") {
				if (this.errorPlugin) {
					//error

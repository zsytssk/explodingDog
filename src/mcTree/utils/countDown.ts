...
export type FuncOnCount = (data: CountInfo) => void;

export class CountDown {
    // 倒计时结束执行函数
    private end_func_list: FuncVoid[] = [];
    // 每次计数都会执行函数
    private count_func_list: FuncOnCount[] = [];
    ...
    */
    ...
    private runCountDown(time) {
        ...
        const show_delta = info.show_delta; // 显示倒计时 变化间隔
        const delta = info.delta; // 实际倒计时 变化间隔
     ...
     * @param fun 绑定函数
     */
    public onCount(fun: FuncOnCount): CountDown {

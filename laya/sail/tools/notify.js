...
;(function () {
    ...
    (function (_super) {
        ...
        _proto.addData = function (data) {
            ...
            if (!this.CONFIG.repeat) {
                for (var i in this.NotifyList) {
                    if (this.NotifyList[i] === html) { return; }
                }
            }

            this.NotifyList.push(html);
        }
        _proto.createNotify = function (type) {
            ...
            while (this.NotifyList.length && limit) {
                this.notifyPanel.addNotify(NotifyItem.create(this.NotifyList.shift()));
                limit--;
            }
        }
        _proto.next = function () {
            if (this.notifyPanel.numChildren === 0 && this.NotifyList.length === 0) {
